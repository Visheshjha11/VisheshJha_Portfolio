export function Projects() {
  return (
    <section
      id="projects"
      className="py-24 bg-neo-yellow border-t-4 border-black px-4 overflow-hidden"
    >
      <div className="max-w-7xl mx-auto">
        <h2
          className="text-6xl md:text-9xl font-black mb-16 uppercase tracking-tighter text-white drop-shadow-[4px_4px_0_rgba(0,0,0,1)] text-stroke-black"
          style={{ WebkitTextStroke: "3px black" }}
        >
          Selected Works
        </h2>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          <article className="reveal group bg-white border-4 border-black p-4 shadow-hard">
            <div className="bg-black border-2 border-black aspect-video relative overflow-hidden mb-6 group-hover:shadow-none transition-all">
              <img
                src="Prepx.png"
                alt="AI Learning Platform"
                className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity"
              />
            </div>
            <div className="flex justify-between items-start">
              <div>
                <p className="font-mono text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">
                  Full-Stack Web Application
                </p>
                <h3 className="text-4xl font-black uppercase mb-2 group-hover:text-neo-red transition-colors glitch-hover">
                  PrepX: AI Learning Platform
                </h3>
                <p className="font-mono text-sm mb-2 max-w-sm">
                  Designed and built a modern learning platform with interactive modules, assessments, and leaderboard-based engagement. Implemented dynamic content delivery, responsive UI, and scalable architecture to support future AI-driven personalization and analytics.
                </p>
                <p className="font-mono text-xs font-bold mb-4 text-neo-black">
                  Focus: Product design, scalability, performance
                </p>
                <div className="flex gap-2 font-mono text-xs font-bold flex-wrap">
                  <span className="bg-neo-black text-white px-2 py-1">React</span>
                  <span className="bg-neo-black text-white px-2 py-1">Next.js</span>
                  <span className="bg-neo-black text-white px-2 py-1">Node.js</span>
                  <span className="bg-neo-black text-white px-2 py-1">MySQL</span>
                  <span className="bg-neo-black text-white px-2 py-1">Tailwind</span>
                </div>
              </div>
              <a
                href="https://prep-x.vercel.app/"
                className="w-12 h-12 flex-shrink-0 border-2 border-black bg-neo-green flex items-center justify-center hover:bg-black hover:text-white transition-all cursor-hover shadow-hard-sm ml-4"
              >
                <i className="ri-arrow-right-up-line text-2xl"></i>
              </a>
            </div>
          </article>

          <article className="reveal group bg-white border-4 border-black p-4 shadow-hard mt-0 md:mt-20">
            <div className="bg-black border-2 border-black aspect-video relative overflow-hidden mb-6 group-hover:shadow-none transition-all">
              <img
                src="./assets/veronica.png"
                alt="Veronica — Personal AI Assistant"
                className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity"
              />
            </div>
            <div className="flex justify-between items-start">
              <div>
                <p className="font-mono text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">
                  AI System / Automation
                </p>
                <h3 className="text-4xl font-black uppercase mb-2 group-hover:text-neo-blue transition-colors glitch-hover">
                  Veronica
                </h3>
                <p className="font-mono text-sm mb-2 max-w-sm">
                  Developed a personal AI assistant capable of acting as a productivity agent and system controller. The assistant handles contextual conversations, task automation, and integrates with external APIs to assist in daily workflows.
                </p>
                <p className="font-mono text-xs font-bold mb-4 text-neo-black">
                  Focus: Machine learning, API integration, Prompt engineering, system design, intelligent behavior
                </p>
                <div className="flex gap-2 font-mono text-xs font-bold flex-wrap">
                  <span className="bg-neo-black text-white px-2 py-1">Python</span>
                  <span className="bg-neo-black text-white px-2 py-1">AI APIs</span>
                  <span className="bg-neo-black text-white px-2 py-1">Automation Scripts</span>
                </div>
              </div>
              <a
                href="https://github.com/visheshjha11/Veronica"
                className="w-12 h-12 flex-shrink-0 border-2 border-black bg-neo-green flex items-center justify-center hover:bg-black hover:text-white transition-all cursor-hover shadow-hard-sm ml-4"
              >
                <i className="ri-arrow-right-up-line text-2xl"></i>
              </a>
            </div>
          </article>

          <article className="reveal group bg-white border-4 border-black p-4 shadow-hard">
            <div className="bg-black border-2 border-black aspect-video relative overflow-hidden mb-6 group-hover:shadow-none transition-all">
              <img
                src="PULSE_img.png"
                alt="PULSE — Wearable Emergency Device"
                className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity"
              />
            </div>
            <div className="flex justify-between items-start">
              <div>
                <p className="font-mono text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">
                  IoT + Embedded Systems
                </p>
                <h3 className="text-4xl font-black uppercase mb-2 group-hover:text-neo-pink transition-colors glitch-hover">
                  PULSE
                </h3>
                <p className="font-mono text-sm mb-2 max-w-sm">
                  Built a wearable emergency response device using an ESP32-based architecture. The system monitors vital signs and motion data, triggering alerts via GSM communication in critical scenarios. Designed for reliability, low power consumption, and real-world use cases.
                </p>
                <p className="font-mono text-xs font-bold mb-4 text-neo-black">
                  Focus: Hardware-software integration, real-time systems
                </p>
                <div className="flex gap-2 font-mono text-xs font-bold flex-wrap">
                  <span className="bg-neo-black text-white px-2 py-1">ESP32</span>
                  <span className="bg-neo-black text-white px-2 py-1">Embedded C/C++</span>
                  <span className="bg-neo-black text-white px-2 py-1">Sensors</span>
                  <span className="bg-neo-black text-white px-2 py-1">GSM</span>
                </div>
              </div>
              <a
                href="https://github.com/visheshjha11/Pulse"
                className="w-12 h-12 flex-shrink-0 border-2 border-black bg-neo-green flex items-center justify-center hover:bg-black hover:text-white transition-all cursor-hover shadow-hard-sm ml-4"
              >
                <i className="ri-arrow-right-up-line text-2xl"></i>
              </a>
            </div>
          </article>
        </div>

        <div className="text-center mt-24">
          <a
            href="https://github.com/Visheshjha11?tab=repositories"
            className="inline-block bg-neo-black text-white px-12 py-5 font-bold font-mono text-xl hover:bg-neo-white hover:text-black border-4 border-black transition-all shadow-hard hover:shadow-none cursor-hover"
          >
            VIEW ALL REPOS ON GITHUB
          </a>
        </div>
      </div>
    </section>
  );
}
